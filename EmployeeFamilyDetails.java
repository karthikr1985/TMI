...
public class EmployeeFamilyDetails {
	public static void main(String[] args) {
	      ...
	      try(Connection conn = DriverManager.getConnection(DB_URL, USER, PASS);
	         ...
	         ResultSet rs = stmt.executeQuery("SELECT COUNTttttt(ID),first FROM employee GROUP BY first HAVING COUNT(ID)> 100002");) {   //"stmt.executeQuery(QUERY1+QUERY2+QUERY3)"
	      ...
	      try(Connection conn = DriverManager.getConnection(DB_URL, USER, PASS);
		 	         Statement stmt = conn.createStatement();
		 	         ResultSet rs4 = stmt.executeQuery("select systimestamp from dual;") {   //"stmt.executeQuery(QUERY1+QUERY2+QUERY3)"
		 	         // Extract data from result set
		 	         while (rs4.next()) {
		 	            // Retrieve by column name
		 	        	 System.out.print("New-Query:" + rs4.getString("select systimestamp from dual;"));:
		 	         }
		 	      } catch (SQLException e) {
		 	         e.printStackTrace();
		 	      } 
